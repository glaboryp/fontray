...
describe('ResultsPage', () => {
  ...
  it('renders font cards when fonts are provided', async () => {
    const fonts = [
      { name: 'Roboto', similarity: 100, link: 'https://x', preview: null, category: 'Sans', foundry: null },
      { name: 'Lato', similarity: 95, link: 'https://y', preview: null, category: 'Sans', foundry: null },
    ]
  ...
  it('sorts fonts by name when selected', async () => {
    const fonts = [
      { name: 'Zebra', similarity: 10, link: null, preview: null, category: 'A', foundry: null },
      { name: 'Alpha', similarity: 90, link: null, preview: null, category: 'A', foundry: null },
    ]
    const encoded = encodeURIComponent(JSON.stringify(fonts))
    window.history.pushState({}, '', `/results?fonts=${encoded}`)
  ...
  it('sorts fonts by similarity when selected', async () => {
    const fonts = [
      { name: 'A', similarity: 10, link: null, preview: null, category: 'A', foundry: null },
      { name: 'B', similarity: 90, link: null, preview: null, category: 'A', foundry: null },
    ]
    const encoded = encodeURIComponent(JSON.stringify(fonts))
    window.history.pushState({}, '', `/results?fonts=${encoded}`)
